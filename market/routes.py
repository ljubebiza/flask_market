    ...
    if request.method == "POST":
        ...
        if s_item_object:
            if current_user.can_sell(s_item_object):
                ...
                flash(f"Congratulations! You have sold {p_item_object.name} back to Market!", category="success")
            else:
        ...
        return render_template("market.html", items=items, buy_form=buy_form, owned_items=owned_items, selling_form=selling_form)
...
def login_page():
    ...
    if form.validate_on_submit():
        ...
        if attemped_user  and attemped_user.check_password_correction(
            ...
            flash("Username and password do not match. Please try agan.", category="danger")
